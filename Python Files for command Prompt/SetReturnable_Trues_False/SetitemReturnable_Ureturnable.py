...
def main():
    ...
    while True:
        ...
        print("====================")
        print("1. Set ALL inventory items returnable/non-returnable")
        ...
        if choice == "1":
            ...
            for item in all_items:
                ...
                print(f"{status} | {item['name']} (SKU: {item['sku']}) | Returnable: {returnable}")
                logs.append({"timestamp": timestamp, "sku": item["sku"], "item_name": item["name"], "status": status})

            log_results(logs)
            print("\n✅ All inventory items processed.")
