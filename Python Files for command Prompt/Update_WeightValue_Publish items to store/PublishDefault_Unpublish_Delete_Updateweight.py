...
def fetch_all_products(token, page=1, per_page=50, active_status=None):
    headers = {
        "Authorization": f"Zoho-oauthtoken {token}",
        "X-com-zoho-store-organizationid": ORG_ID
    }
    url = f"{API_BASE_URL}/products?page={page}&per_page={per_page}"
    if active_status:  # filter by status active/inactive if provided
        url += f"&status={active_status}"
...
def delete_product(com_token, product_id):
    url = f"{API_BASE_URL}/products/{product_id}"
    headers = {
        "Authorization": f"Zoho-oauthtoken {com_token}",
        "X-com-zoho-store-organizationid": ORG_ID,  # ✅ add this
        "Content-Type": "application/json"
    }
    ...
    if resp.status_code == 200:
        ...
        print(f"❌ Failed to delete product ID {product_id}. Status: {resp.status_code}, Response: {resp.text}")
...
def get_all_skus():
    ...
    page = 1
    while True:
        products, page_context = fetch_all_products(com_token, page=page)
        for prod in products:
            if prod.get("variants"):
                for var in prod["variants"]:
                    all_skus.append(var.get("sku", "").upper())
            else:
                all_skus.append(prod.get("sku", "").upper())
        if not page_context.get("has_more_page"):
            break
        page += 1
    return all_skus

def get_skus_by_status():
    com_token = get_zoho_commerce_token()
    choice = input("Fetch Active items? (y/n): ").strip().lower()
    status = "active" if choice == "y" else "inactive"

    all_items = []  # store tuples (index, name, sku)
    page = 1
    while True:
        products, page_context = fetch_all_products(com_token, page=page, active_status=status)
        for prod in products:
            if prod.get("variants"):
                for var in prod["variants"]:
                    sku = var.get("sku", "").upper()
                    name = f"{prod.get('name')} - {var.get('name')}" if var.get("name") else prod.get("name")
                    all_items.append((len(all_items) + 1, name, sku))
            else:
    ...
    # Print numbered list
    print(f"\nFetched {len(all_items)} {status} products:\n")
    for idx, name, sku in all_items:
        print(f"{idx}. {name} [{status}] (SKU: {sku})")

    if not all_items:
        return [], None

    toggle = input(f"\nDo you want to toggle them to {'inactive' if status=='active' else 'active'}? (y/n): ").strip().lower()
    if toggle == 'y':
        prod_choice = input("Enter number(s) or SKU(s) (comma separated) or 'all': ").strip()

        if prod_choice.lower() == "all":
            return [sku for _, _, sku in all_items], (status != "active")

        selected_skus = []
        for token in prod_choice.split(","):
            token = token.strip()
            if token.isdigit():  # user entered index number
                idx = int(token)
                if 1 <= idx <= len(all_items):
                    selected_skus.append(all_items[idx - 1][2])  # pick SKU
                else:
                    print(f"❌ Invalid number: {token}")
            else:  # assume user entered SKU
                matches = [sku for _, _, sku in all_items if sku == token.upper()]
                if matches:
                    selected_skus.extend(matches)
                else:
                    print(f"❌ SKU not found: {token}")

        return selected_skus, (status != "active")

    return [sku for _, _, sku in all_items], None
...
def get_product_id_by_sku(sku, com_token):
    """
    Fetch all commerce products and return the product_id for a given SKU.
    ...
    while True:
        ...
        for prod in products:
            ...
            if variants:
                for var in variants:
                    if var.get("sku", "").upper() == sku.upper():
                        return parent_id
            else:
    ...
    print(f"❌ SKU not found in Commerce: {sku}")
    return None

   
def update_inventory_weight(access_token, item_id, new_weight):
    """Update the weight of an inventory item in Zoho Inventory (including package_details)."""
    ...
    data = {
        "weight": new_weight,
        "weight_unit": "kg",
        "weight_with_unit": f"{new_weight} kg",
        "package_details": {
            "weight": new_weight,
            "weight_unit": "kg"
        }
    ...
    if response.status_code == 200:
        print(f"✅ Inventory weight updated to {new_weight} kg (including package details).")
    else:
        print(f"❌ Failed to update inventory: {response.status_code} {response.text}")
...
def main():
    while True:
        ...
        if mode == '1':
            ...
            pad_len = int(input("Enter padding length: ").strip())
            ...
            sku_list, toggle_flag = get_skus_by_status()
        else:
        ...
        print("\nChoose action:\n1 - Publish only Active Items\n2 - Unpublish\n3 - Set Active/Inactive only\n4 - Set Active and Publish\n5 - Delete Items (Admin only)\n6 - Update Weight")
        while True:
            choice = input("Enter choice (1, 2, 3, 4, 5, or 6): ").strip()
            if choice == '1':
                show_flag, active_flag = True, None
                break
            elif choice == '2':
                show_flag, active_flag = False, None
                break
            elif choice == '3':
                af_choice = input("Set product Active? (y/n): ").strip().lower()
                show_flag, active_flag = None, (True if af_choice == 'y' else False)
                break
            elif choice == '4':
                show_flag, active_flag = True, True  # Set Active and Publish
                break
            elif choice == '5':
                password = input("Enter Admin password to proceed with deletion: ").strip()
                if password != "Admin123":
                    print("❌ Incorrect password. Deletion cancelled.")
                else:
                    confirm = input("⚠️ Are you sure you want to permanently delete these items? (yes/no): ").strip().lower()
                    if confirm == "yes":
                        com_token = get_zoho_commerce_token()
                        parents = map_inventory_to_commerce(sku_list, com_token)
                        print("\nCommerce Products for Selected SKUs:")
                        ...
                        choice_nums = input("\nChoose parent or variant numbers to delete (e.g., 1,2 or 1.1,1.2 for variants, 'all' for all): ").strip()
                        to_delete = []

                        if choice_nums.lower() == "all":
                            for pid, pdata in parents.items():
                                for var in pdata['variants']:
                                    to_delete.append((pid, var['variant_id'], var['sku']))
                        else:
                            selections = choice_nums.split(',')
                            for sel in selections:
                                parts = sel.strip().split('.')
                                parent_idx = int(parts[0]) - 1
                                parent_key = list(parents.keys())[parent_idx]
                                ...
                                if len(parts) == 2:
                                    var_idx = int(parts[1]) - 1
                                    var = pdata['variants'][var_idx]
                                    to_delete.append((parent_key, var['variant_id'], var['sku']))
                                else:
                                    for var in pdata['variants']:
                                        to_delete.append((parent_key, var['variant_id'], var['sku']))

                        for pid, vid, sku in to_delete:
                            delete_product(com_token, pid)
                    else:
                        print("❌ Deletion cancelled.")
                show_flag, active_flag = None, None
                break
            elif choice == '6':
                inv_token = get_zoho_inventory_token()
                if not inv_token:
                    print("❌ Could not get Inventory token.")
                    break

                new_weight = float(input("Enter new weight (kg): ").strip())

                # Map SKUs to inventory items
                for sku in sku_list:
                    ...
                    if "items" in data and data["items"]:
                        ...
                        update_inventory_weight(inv_token, item_id, new_weight)
                    else:
        ...
        if toggle_flag is not None:
            show_flag, active_flag = None, toggle_flag

        if show_flag is not None or active_flag is not None:
    ...
    while True:
        ...
        while True:
            ...
            if choice == '1':
                ...
                show_flag, active_flag = True, True
                break
            elif choice == '5':
                password = input("Enter Admin password to proceed with deletion: ").strip()
                if password != "Admin123":
                ...
                choice_nums = input(
                    "\nChoose parent or variant numbers to update weight "
                    "(e.g., 1,2 or 1.1,1.2 for variants, 'all' for all): "
                ).strip()
                ...
                # Ask if one weight should apply to all
                if len(to_update) > 1:
                    same_weight = input("Apply one weight value to all selected items? (y/n): ").strip().lower()
                else:
                    same_weight = "n"

                if same_weight == "y":
                    one_weight = input("Enter weight (kg) to apply to all: ").strip()
                    try:
                        one_weight = float(one_weight)
                    except:
                        print("❌ Invalid weight input. Operation cancelled.")
                        break
                    for pid, vid, sku, pname, vname in to_update:
                        url = f"https://inventory.zoho.com/api/v1/items?sku={sku}&organization_id={ORG_ID}"
                        headers = {"Authorization": f"Zoho-oauthtoken {inv_token}"}
                        ...
                        if "items" in data and data["items"]:
                            item_id = data["items"][0]["item_id"]
                            update_inventory_weight(inv_token, item_id, one_weight)
                        else:
                            print(f"❌ SKU not found in Inventory: {sku}")
                else:
                    for pid, vid, sku, pname, vname in to_update:
                        new_weight = input(f"Enter new weight (kg) for {pname} - {vname if vname else ''} (SKU: {sku}): ").strip()
                        try:
                            new_weight = float(new_weight)
                        except:
                            print(f"❌ Invalid weight input for SKU {sku}. Skipping.")
                            continue
