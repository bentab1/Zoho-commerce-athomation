...
# --- Select mode (mandatory loop) ---
    while True:
        print("\nStock Adjustment Mode:")
        print("1 - Add/Subtract Stock")
        print("2 - Set Stock Level")
        choice = input("Enter choice (1 or 2) or 'exit': ").strip()
...
def adjust_stock(com_token, pid, vid, sku, pname, vname, desired_level, mode, reason, inv_token, index=None):
    ...
    if quantity == 0:
        print(f"⚠️ [{sku}]: Skipped (Already at target {desired_level})")
        return
...
def main():
    ...
    while True:  # main menu loop
        ...
        # --- Apply Adjustment To All Inventory Items ---
        elif mode == "3":
            apply_all = input("Apply Adjustment To all inventory items? (y/n): ").strip().lower()
            if apply_all != "y":
                continue
            ...
            while True:
                ...
                for item in items:
                    if "sku" in item and item["sku"]:
                        sku_list.append(item["sku"].upper())
            ...
            print(f"✅ Found {len(sku_list)} SKUs in inventory.")
