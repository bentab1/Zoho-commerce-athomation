...
def select_category(access_token, current_category=""):
    ...
    for idx, c in enumerate(cat_names, 1):
        print(f"{idx}. {c}")
    while True:
        user_input = input(f"Category [{current_category}]: ").strip()
        if not user_input: return current_category, None
        if user_input.isdigit():
            idx = int(user_input)
            if 1 <= idx <= len(cat_names):
                return cat_names[idx-1], cat_map[cat_names[idx-1]]
            print("❌ Invalid number.")
            continue
        matches = [name for name in cat_names if name.lower() == user_input.lower()]
        if matches:
            return matches[0], cat_map[matches[0]]
        print("❌ Invalid category. Type a valid one or press Enter to skip.")
...
def select_brand(access_token, current_brand=""):
    ...
    for idx, b in enumerate(brand_names, 1):
        print(f"{idx}. {b}")
    while True:
        user_input = input(f"Brand [{current_brand}]: ").strip()
        if not user_input: return current_brand
        if user_input.isdigit():
            idx = int(user_input)
        ...
        print("❌ Invalid brand. Type a valid one or press Enter to skip.")
    ...
    while True:
        ...
        if mode_choice == "1":
            ...
            for sku in skus:
                ...
                log_rows.append({"SKU": sku, "Status": status, "Category": category_name or "Unchanged", "Brand": brand or "Unchanged", "Unit": unit or "Unchanged"})
                print(f"{'✅' if status_code==200 else '❌'} Updated '{product.get('name')}'")
