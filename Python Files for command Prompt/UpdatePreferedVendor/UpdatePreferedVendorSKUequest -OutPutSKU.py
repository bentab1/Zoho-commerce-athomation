...
def get_sku_list():
    ...
    if mode == "1":
        ...
        # 🔹 Show generated SKUs in comma-separated form
        print("\n📦 Generated SKUs:")
        print(", ".join(skus))
        ...
        # 🔹 Show provided SKUs in comma-separated form
        print("\n📦 Provided SKUs:")
        print(", ".join(skus))
...
def main():
    ...
    # 🔹 Print SKUs not found in groups of 5 per line
    if not_found_skus:
        print("\n❌ The following SKUs were NOT found in inventory:")
        group_size = 5
        for i in range(0, len(not_found_skus), group_size):
            group = not_found_skus[i:i+group_size]
            print(", ".join(group))
    # --- Prompt to repeat ---
    while True:
