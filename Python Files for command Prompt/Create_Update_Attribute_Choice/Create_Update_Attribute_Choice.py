...
#     """
#     Add or update an attribute for a product using Zoho Commerce /variants API.
#     - attr_name: string, e.g., "Color"
#     - choices: list of values, e.g., ["Red", "Blue"]
#     """
...
#         if not variants:
#             print(f"⚠️ Product {product_id} has no variants to update.")
#             return False
#     except Exception as e:
...
#     # 2️⃣ Update existing variants with the attribute
#     for variant in variants:
#         variant_id = variant.get("id")
#         if not variant_id:
#             continue

#         # Skip if attribute already exists with same value
#         for choice in choices:
#             attr_exists = any(a.get("name") == attr_name and a.get("value") == choice for a in variant.get("attributes", []))
#             if attr_exists:
#                 continue

#             # Prepare payload for PUT /variants/{variant_id}
#             payload = {
#                 "variant": {
#                     "attributes": variant.get("attributes", []) + [{"name": attr_name, "value": choice}],
#                     "sku": f"{variant.get('sku')}-{choice.replace(' ', '').upper()}",
#                     "name": f"{variant.get('name')} - {choice}"
#                 }
#             }

#             try:
#                 put_resp = requests.put(
#                     f"{API_BASE_URL}/variants/{variant_id}",
#                     headers=headers,
#                     params={"organization_id": ORG_ID},
#                     json=payload
#                 )
...
#                     success = False
#                 else:
#                     print(f"✅ Updated variant {variant_id} with {attr_name}: {choice}")
#             except Exception as e:
#                 print(f"❌ Exception updating variant {variant_id}: {e}")
#                 success = False

#     # 3️⃣ Optionally: create new variants for any choice combinations not covered
...
def create_or_update_attribute(token, product_id, attr_name, choices):
    ...
    for variant in variants:
        ...
        for choice in choices:
            ...
            # Prepare payload for PUT /variants/{variant_id} WITHOUT changing SKU
            payload = {
                "variant": {
                    "attributes": variant.get("attributes", []) + [{"name": attr_name, "value": choice}],
                    "name": f"{variant.get('name')} - {choice}"
...
def update_product_title(token, product_id, new_title):
    """
    Update a Zoho Commerce product's title.
    Returns True if successful, False otherwise.
    """
    url = f"{API_BASE_URL}/products/{product_id}"
    headers = {"Authorization": f"Zoho-oauthtoken {token}", "Content-Type": "application/json"}
    payload = {"product": {"name": new_title}}
    try:
        resp = requests.put(url, headers=headers, params={"organization_id": ORG_ID}, json=payload)
