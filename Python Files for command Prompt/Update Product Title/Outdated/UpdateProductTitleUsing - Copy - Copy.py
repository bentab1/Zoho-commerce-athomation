...
def update_product_title(headers, product, update_type, old_words=None, new_words=None, new_title_override=None, append_text=None):
    ...
    # Skip if append_text already exists
    if update_type == "3" and append_text and append_text in parent_title:
        log(f"'{append_text}' already in title for {product.get('sku')}, skipping...")
        return
    ...
    parent_data = {"name": new_parent_title, "url": parent_url_slug}
    ...
    for variant in product.get("variants", []):
        ...
        v_data = {"name": new_variant_name, "url": slugify(new_variant_name)}
...
def update_product_titles(token):
    ...
    updated_products = set()  # Track updated product IDs
    ...
    while True:
        ...
        if update_type == "1":
            ...
            new_title_override = input("New title or replacements (use old->new,comma): ").strip()
        elif update_type == "3":
        ...
        if choice == "1":
            # 🔒 Ask for admin password before bulk updating
            password = input("Enter admin password to update ALL products: ").strip()
            if password != "Admin123":
                error("❌ Incorrect admin password. Exiting.")
                sys.exit(1)

            # Bulk update with confirmation
            products = fetch_all_products(headers)
            for product in products:
                if product.get("product_id") in updated_products:
                    log(f"Skipping already updated product {product.get('sku')}")
                    continue
                ...
                if confirm == 'y':
                    ...
                    updated_products.add(product.get("product_id"))

        elif choice == "2":
            # Specific SKUs without confirmation
            skus = get_sku_list()
            for sku in skus:
                ...
                for product in products:
                    ...
                    if full_details:
                        log(f"\nUpdating product: {full_details.get('sku')} - {full_details.get('name')}")
                        update_product_title(headers, full_details, update_type, old_word, new_word, new_title_override, append_text)
                        updated_products.add(full_details.get("product_id"))
        else:
